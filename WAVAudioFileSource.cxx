...
WAVAudioFileSource1*
WAVAudioFileSource1::createNew(UsageEnvironment& env, char const* fileName) {
  do {
    FILE* fid = OpenInputFile(env, fileName);
    if (fid == NULL) break;

    WAVAudioFileSource1* newSource = new WAVAudioFileSource1(env, fid);
	/*
    if (newSource != NULL && newSource->bitsPerSample() == 0) {
      // The WAV file header was apparently invalid.
	  //printf("newSource PROBLEM %s\n", newSource);
...
unsigned WAVAudioFileSource1::numPCMBytes() const {
  if (fFileSize < fWAVHeaderSize) return 0;
  return fFileSize - fWAVHeaderSize;
}

void WAVAudioFileSource1::setScaleFactor(int scale) {
...
void WAVAudioFileSource1::seekToPCMByte(unsigned byteNumber) {
...
void WAVAudioFileSource1::limitNumBytesToStream(unsigned numBytesToStream) {
  fNumBytesToStream = numBytesToStream;
  fLimitNumBytesToStream = fNumBytesToStream > 0;
}

unsigned char WAVAudioFileSource1::getAudioFormat() {
  return fAudioFormat;
}
...
WAVAudioFileSource1::WAVAudioFileSource1(UsageEnvironment& env, FILE* fid)
...
WAVAudioFileSource1::~WAVAudioFileSource1() {
  if (fFid == NULL) return;
...
void WAVAudioFileSource1::doGetNextFrame() {
...
void WAVAudioFileSource1::doStopGettingFrames() {
...
void WAVAudioFileSource1::fileReadableHandler(WAVAudioFileSource1* source, int /*mask*/) {
...
void WAVAudioFileSource1::doReadFromFile() {
...
Boolean WAVAudioFileSource1::setInputPort(int /*portIndex*/) {
  return True;
}

double WAVAudioFileSource1::getAverageLevel() const {
